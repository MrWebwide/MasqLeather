...
$klasord="resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into cupon set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,fiyat=:fiyat,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from cupon order by id desc")->fetch(PDO::FETCH_ASSOC);
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM cupon WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update cupon set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,fiyat=:fiyat,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from cupon where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
									...
									<a href="cupon-listele.php"  class="btn btn-primary m-b-md">List Category</a>
                                    ...
                                    <div class="form-floating mb-3">
									  ...
									  <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">Coupon Discount </label>
                                      </div>
