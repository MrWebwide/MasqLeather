...
$klasord="../resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into etiket set adi=:adi,sira=:sira,resim=:resim,linki=:linki,durum=:durum,aciklama=:aciklama");
	...
	if($ekle){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Ekip Üstü Başarıyla Eklendi!</strong> 
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM etiket WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	else
	{
	 ...
	 $simdi1 = $db->prepare("update etiket set adi=:adi,sira=:sira,resim=:resim,linki=:linki,durum=:durum,
	...
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Ekip Üstü Başarıyla Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from etiket where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Ekip Üstü Ekle - <?=$ayar['site_title']?></title>
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-body table-responsive">
                                    <h5 class="card-title">Ekip Üstü Ekle</h5>
                                        <a href="etiket-listele.php"  class="btn btn-primary m-b-md">Ekip Üstü Listele</a>
                                    <p class="card-description">Please fill all of the available spaces.</p>
                                    <?=$mesaj?>
                                    <form method="post" enctype="multipart/form-data" >
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="sira" placeholder="Ekip Üstü Sırası" value="<?=$guncelle['sira']?>">
                                        <label for="floatingInput">Ekip Üstü sırası</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Ekip Üstü Adı" value="<?=$guncelle['adi']?>">
                                        <label for="floatingInput">Ekip Üstü Adı</label>
                                      </div>
                                      
                                      <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="linki" placeholder="Ekip Üstü Linki" value="<?=$guncelle['linki']?>">
                                        <label for="floatingInput">Ekip Üstü Linki</label>
                                      </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                        <label for="formFile" class="form-label">Ekip Üstü Resmi</label>
