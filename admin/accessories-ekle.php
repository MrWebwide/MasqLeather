...
if(empty($resim_tmpd1)) {
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from accessories order by id desc")->fetch(PDO::FETCH_ASSOC);
				
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
    	foreach ($_POST['img'] as $img) {
    		$islem = $db->prepare("INSERT INTO accessories_img SET urun_id = ?, img = ?,tur=?");
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
     ...
     // Check if a new video file is uploaded
		if (!empty($_FILES['video']['tmp_name'])) {
			...
			if (
				...
				$ayar_kaydi = $db->query("SELECT * FROM accessories WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
    ...
    if(empty($resim_tmp1)) {
        ...
        if ($_FILES["resim1"]["type"] =="image/gif" || $_FILES["resim1"]["type"] =="image/png" || $_FILES["resim1"]["type"] =="image/jpg" || $_FILES["resim1"]["type"] =="image/jpeg") {
		  ...
		  $deleteee = $db->exec("DELETE FROM accessories_img WHERE urun_id = '$id' ");
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from accessories where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
if (isset($_GET['islem']) && $_GET['islem'] == 'duzenle') {
    ...
    $stmt = $db->prepare("SELECT sira FROM accessories WHERE id = ?");
    ...
    $stmt = $db->query("SELECT MAX(sira) AS max_sira FROM accessories");
...
$query = $db->query("SELECT durum FROM accessories WHERE id='$id'");
                        ...
                        <div class="col">
                            <div class="card">
								...
								<h5 class="card-title">Add Product</h5>
								   ...
								   <a href="accessories-listele.php"  class="btn btn-primary m-b-md">List Products</a> <!-- Ürünü kopyalamak için bir buton -->
...
<div class="mb-3">
    ...
    <select class="form-select" name="yazi10" onchange="updateImage(this)">
        ...
        <?php
        $urun_kategori = $db->query("SELECT * FROM accessories ORDER BY id DESC", PDO::FETCH_ASSOC);
        if ($urun_kategori->rowCount()) {
            foreach ($urun_kategori as $urunkat) {
                ?>
					  ...
					  <?php }}?>
						 ...
						 <br>
                            	...
                            	<?php
									...
									$iddd=intval($_GET['id']);
									if($_GET['islem']=='duzenle'){
										$cek = $db->query("SELECT * FROM accessories_img WHERE urun_id = '$iddd' order by id asc", PDO::FETCH_ASSOC);
										if($cek->rowCount()){
											foreach( $cek as $c ){
...
document.getElementById("productCopy").addEventListener("click", function(event){
    ...
    xhr.open("GET", "copy_product_accessories.php?id=" + productId, true);
