...
$klasord="resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into sc set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from sc order by id desc")->fetch(PDO::FETCH_ASSOC);
...
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Portfoy Başarıyla Eklendi!</strong> 
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM sc WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update sc set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	...
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Portfoy Başarıyla Güncellendi!</strong>
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from sc where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>SC Ekle | <?=$ayar['site_title']?></title>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Yorum  Ekle</h5>
                                      <a href="sc-listele.php"  class="btn btn-primary m-b-md">Yorum Listele</a>
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">Comment Order</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Product Name" value="<?=$guncelle['adi']?>">
                                        <label for="floatingInput">Comment Name</label>
                                      </div>
								
									
									  <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="kategori" placeholder="Product Name" value="<?=$guncelle['kategori']?>">
                                        <label for="floatingInput">Comment Order City</label>
                                      </div>
									  <div class=" mb-3">
                                        ...
                                        <div class="mb-3">
                                        <label for="formFile" class="form-label">Comment Picture</label>
