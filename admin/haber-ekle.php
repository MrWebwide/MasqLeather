...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update haberler set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,yazi1=:yazi1,yazi2=:yazi2,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"yazi1"=>$yazi1,"yazi2"=>$yazi2,"guncelleme_tarihi"=>$tarih,"id"=>$id));
	if($ekle1){
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from haberler where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Referans  Ekle</h5>
                                      <a href="haber-listele.php"  class="btn btn-primary m-b-md">List Blog</a>
                                    <p class="card-description">Please fill all of the available spaces.</p>
                                    <?=$mesaj?>
                                    <form method="post" enctype="multipart/form-data" >
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="sira" placeholder="Blog  Sırası" value="<?=$guncelle['sira']?>">
                                        <label for="floatingInput">Referans sırası</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Blog Name" value="<?=$guncelle['adi']?>">
                                        <label for="floatingInput">Referans Adı</label>
                                      </div>
									
									  <div class="mb-3">
                                        <label for="formFile" class="form-label">Referans Resmi</label>
									...
									<!--
									  <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="yazi1" placeholder="Blog Konusu" value="<?=$guncelle['yazi1']?>">
                                        <label for="floatingInput">Referans Konusu</label>
                                      </div>
									  <div class="form-floating mb-3">
                            	...
                            	<?php
									...
									$iddd=intval($_GET['id']);
									if($_GET['islem']=='duzenle'){
										$cek = $db->query("SELECT * FROM urun_img WHERE blog_id = '$iddd' order by id asc", PDO::FETCH_ASSOC);
										if($cek->rowCount()){
											foreach( $cek as $c ){
