...
$klasord="resimler/";
	...
	$simdi = $db->prepare("insert into amenu set adi=:adi,sira=:sira,resim=:resim,resim1=:resim1,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,yazi2=:yazi2,yazi1=:yazi1,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	$ekle = $simdi->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"resim1"=>$resim1,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"yazi2"=>$yazi2,"yazi1"=>$yazi1,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"eklenme_tarihi"=>$tarih));
	if($ekle){
		
		
		
		  $sonid=$db->query("select * from amenu order by id desc")->fetch(PDO::FETCH_ASSOC);
...
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Anasayfa Menü Başarıyla Eklendi!</strong>
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM amenu WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update amenu set adi=:adi,sira=:sira,resim=:resim,resim1=:resim1,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,yazi2=:yazi2,yazi1=:yazi1,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"resim1"=>$resim1,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"yazi2"=>$yazi2,"yazi1"=>$yazi1,"guncelleme_tarihi"=>$tarih,"id"=>$id));
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong> Anasayfa Menü Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from amenu where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Anasayfa Menü Ekle | <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Anasayfa Menü  Ekle</h5>
                                      <a href="amenu-listele.php"  class="btn btn-primary m-b-md">Anasayfa Menü Listele</a>
                                    <p class="card-description">Please fill all of the available spaces.</p>
                                    <?=$mesaj?>
                                    <form method="post" enctype="multipart/form-data" >
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="sira" placeholder="Özel Menü  Sırası" value="<?=$guncelle['sira']?>">
                                        <label for="floatingInput"> Menü sırası</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Özel Menü Adı" value="<?=$guncelle['adi']?>">
									  ...
									  <div class=" mb-3">
                                            <label for="floatingInput">Menü Preliminary Statement</label>
                                          <textarea class="form-control" name="yazi2" rows="10" cols="100"><?=$guncelle['yazi2']?></textarea>
                                        
                                      </div>

									  <div class="mb-3">
                                        <label for="formFile" class="form-label">Özel Menü Resmi</label>
									  ...
									  <!--
									   ...
									   <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="yazi1" placeholder="Fiyatı" value="<?=$guncelle['yazi1']?>">
								 ...
								 <textarea  class="ckeditor" name="aciklama"  rows="10"><?=$guncelle['aciklama']?></textarea>
									  ...
									  <div class="mb-3">
                                        <label for="formFile" class="form-label">Hizmet Resmi</label>
