...
if ($_POST['kaydet'] && empty($_GET['islem'])) {
    // Yeni kampanya ekle
    $simdi = $db->prepare("insert into jewekampanya set adi=:adi, sira=:sira, resim=:resim, kategori=:kategori, durum=:durum, onaciklama=:onaciklama, fiyat=:fiyat, seo=:seo, tur=:tur, eklenme_tarihi=:eklenme_tarihi");
    ...
    if ($ekle) {
        $sonid = $db->query("select * from jewekampanya order by id desc")->fetch(PDO::FETCH_ASSOC);

        // Yeni kampanya eklendikten sonra urunler tablosunu güncelle
        $updateUrunlerQuery = "UPDATE jewe SET kampanya = :fiyat WHERE kategori = :kategori";
...
if ($_GET['islem'] == 'duzenle') {
    $gid = $_GET['id'];
    $guncelle = $db->query("select * from jewekampanya where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
?>
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
									...
									<a href="jewe-kampanya-listele.php"  class="btn btn-primary m-b-md">List Category</a>
