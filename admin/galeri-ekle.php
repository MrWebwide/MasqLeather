...
imagedestroy($content);
	unlink ('../resimler/'.$resim1);
	 ...
	 $simdi = $db->prepare("insert into galeri set adi=:adi,sira=:sira,resim=:resim,linki=:linki,durum=:durum");
	$ekle = $simdi->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"linki"=>$linki,"durum"=>$durum));
	if($ekle){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Galeri Başarıyla Eklendi!</strong> 
...
unlink ('../resimler/'.$resim1);
	 ...
	 $simdi1 = $db->prepare("update galeri set adi=:adi,sira=:sira,resim=:resim,linki=:linki,durum=:durum where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"linki"=>$linki,"durum"=>$durum,"id"=>$id));
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Galeri Başarıyla Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-body table-responsive">
                                    <h5 class="card-title">Galeri  Ekle</h5>
                                    <a href="galeri-listele.php"  class="btn btn-primary m-b-md">Galeri Listele</a>
                                    <p class="card-description">Please fill all of the available spaces.</p>
                                    <?=$mesaj?>
                                    <form method="post" enctype="multipart/form-data" >
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="sira" placeholder="Galeri Sırası" value="<?=$guncelle['sira']?>">
                                        <label for="floatingInput">Galeri sırası</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Galeri Adı" value="<?=$guncelle['adi']?>">
                                        <label for="floatingInput">Galeri Adı</label>
                                      </div>
                                      
                                      <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="linki" placeholder="Galeri Linki" value="<?=$guncelle['linki']?>">
                                        <label for="floatingInput">Galeri Linki</label>
                                      </div>
