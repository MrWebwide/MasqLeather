...
if ($_POST['kaydet'] && $_GET['islem'] == 'duzenle') {
    ...
    // Kampanya indirimi ürün satırına (homedecor.kampanya) damgalandığı için, kategori
    ...
    $eskiKatQ = $db->prepare("SELECT kategori FROM homedecorkampanya WHERE id = ?");
    $eskiKatQ->execute([$id]);
    $eskiKategori = (string) $eskiKatQ->fetchColumn();
    if ($eskiKategori !== '' && $eskiKategori !== $kategori) {
        $db->prepare("UPDATE homedecor SET kampanya = 0 WHERE kategori = ?")->execute([$eskiKategori]);
    }
