...
$klasord="resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into cargo_kategori set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,fiyat=:fiyat,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from cargo_kategori order by id desc")->fetch(PDO::FETCH_ASSOC);
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM cargo_kategori WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update cargo_kategori set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,fiyat=:fiyat,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from cargo_kategori where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Cargo US Category</h5>
									<a href="cargo-kategori-listele.php"  class="btn btn-primary m-b-md">Cargo US Category List</a>
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">Cargo Group Order</label>
                                      </div>
