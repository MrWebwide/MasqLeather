...
$klasord="resimler/";
	...
	$simdi = $db->prepare("insert into new set adi=:adi,sira=:sira,resim=:resim,durum=:durum,seo=:seo,yazi1=:yazi1,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	$ekle = $simdi->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"seo"=>$seo,"tur"=>$tur,"yazi1"=>$yazi1,"durum"=>$durum,"eklenme_tarihi"=>$tarih));
	if($ekle){
		
		
		
		  $sonid=$db->query("select * from new order by id desc")->fetch(PDO::FETCH_ASSOC);
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM new WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update new set adi=:adi,sira=:sira,resim=:resim,durum=:durum,yazi1=:yazi1,,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"seo"=>$seo,"tur"=>$tur,"durum"=>$durum,"yazi1"=>$yazi1,"guncelleme_tarihi"=>$tarih,"id"=>$id));
	if($ekle1){
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from new where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>ÜrünAdd Product | <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">New Products  Ekle</h5>
                                      <a href="new-listele.php"  class="btn btn-primary m-b-md">New Products Listele</a>
                                    ...
                                    <div class="form-floating mb-3">
									  ...
									  <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">New Products Fiyatı</label>
                                      </div>

									  <div class="mb-3">
                                        <label for="formFile" class="form-label">New Products Resmi</label>
