...
$klasord="resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into portfoy set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM portfoy WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update portfoy set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Galeri  Ekle</h5>
                                      <a href="portfoy-listele.php"  class="btn btn-primary m-b-md">Galeri Listele</a>
