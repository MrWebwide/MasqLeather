...
if (isset($_GET['islem']) && $_GET['islem'] == 'duzenle') {
    ...
    $stmt = $db->prepare("SELECT sira FROM homedecor WHERE id = ?");
    ...
    $stmt = $db->query("SELECT MAX(sira) AS max_sira FROM homedecor");
...
$query = $db->query("SELECT durum FROM homedecor WHERE id='$id'");
                        ...
                        <div class="col">
                            <div class="card">
								...
								<h5 class="card-title">Add Product</h5>
								   ...
								   <a href="homedecor-listele.php"  class="btn btn-primary m-b-md">List Products</a> <!-- Ürünü kopyalamak için bir buton -->
					  ...
					  <?php }}?>
						 ...
						 <br>
                            	...
                            	<?php
									...
									$iddd=intval($_GET['id']);
									if($_GET['islem']=='duzenle'){
										$cek = $db->query("SELECT * FROM home_img WHERE urun_id = '$iddd' order by id asc", PDO::FETCH_ASSOC);
										if($cek->rowCount()){
											foreach( $cek as $c ){
...
document.getElementById("productCopy").addEventListener("click", function(event){
    ...
    xhr.open("GET", "copy_product_homedecor.php?id=" + productId, true);
