...
if ($_POST['kaydet'] && empty($_GET['islem'])) {
    // Yeni kampanya ekle
    $simdi = $db->prepare("insert into kampanya set adi=:adi, sira=:sira, resim=:resim, kategori=:kategori, durum=:durum, onaciklama=:onaciklama, fiyat=:fiyat, seo=:seo, tur=:tur, eklenme_tarihi=:eklenme_tarihi");
    ...
    if ($ekle) {
        $sonid = $db->query("select * from kampanya order by id desc")->fetch(PDO::FETCH_ASSOC);

        // Yeni kampanya eklendikten sonra urunler tablosunu güncelle
        $updateUrunlerQuery = "UPDATE urunler SET kampanya = :fiyat WHERE kategori = :kategori";
...
if ($_POST['kaydet'] && $_GET['islem'] == 'duzenle') {
    ...
    // Kampanya indirimi ürün satırına (urunler.kampanya) damgalandığı için, kategori
    ...
    $eskiKatQ = $db->prepare("SELECT kategori FROM kampanya WHERE id = ?");
    $eskiKatQ->execute([$id]);
    $eskiKategori = (string) $eskiKatQ->fetchColumn();
    if ($eskiKategori !== '' && $eskiKategori !== $kategori) {
        $db->prepare("UPDATE urunler SET kampanya = 0 WHERE kategori = ?")->execute([$eskiKategori]);
    }

    // Kampanya güncelle
    $simdi1 = $db->prepare("update kampanya set adi=:adi, sira=:sira, resim=:resim, kategori=:kategori, durum=:durum, onaciklama=:onaciklama, fiyat=:fiyat, seo=:seo, tur=:tur, guncelleme_tarihi=:guncelleme_tarihi where id=:id");
...
if ($_GET['islem'] == 'duzenle') {
    $gid = intval($_GET['id']);
    $guncelle = $db->query("select * from kampanya where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
?>
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
									...
									<a href="bag-purses-kampanya-listele.php"  class="btn btn-primary m-b-md">List Category</a>
