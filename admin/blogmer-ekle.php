...
$klasord="resimler/";
	...
	$simdi = $db->prepare("insert into bloglarmer set adi=:adi,sira=:sira,resim=:resim,resim1=:resim1,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,yazi1=:yazi1,yazi2=:yazi2,yazi3=:yazi3,yazi4=:yazi4,yazi5=:yazi5,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	$ekle = $simdi->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"resim1"=>$resim1,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"yazi1"=>$yazi1,"yazi2"=>$yazi2,"yazi3"=>$yazi3,"yazi4"=>$yazi4,"yazi5"=>$yazi5,"onaciklama"=>$onaciklama,"durum"=>$durum,"eklenme_tarihi"=>$tarih));
	if($ekle){
		
		
		
		  $sonid=$db->query("select * from bloglarmer order by id desc")->fetch(PDO::FETCH_ASSOC);
				
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
    	foreach ($_POST['img'] as $img) {
    		$islem = $db->prepare("INSERT INTO blog_img SET blog_id = ?, img = ?,tur=?");
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Blog Başarıyla Eklendi!</strong>
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM bloglarmer WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update bloglarmer set adi=:adi,sira=:sira,resim=:resim,resim1=:resim1,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,yazi1=:yazi1,yazi2=:yazi2,yazi3=:yazi3,yazi4=:yazi4,yazi5=:yazi5,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"resim1"=>$resim1,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"yazi1"=>$yazi1,"yazi2"=>$yazi2,"yazi3"=>$yazi3,"yazi4"=>$yazi4,"yazi5"=>$yazi5,"guncelleme_tarihi"=>$tarih,"id"=>$id));
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Blog Başarıyla Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from bloglarmer where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Add Blog | <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Blog Mercantaile Ekle</h5>
                                      <a href="blogmer-listele.php"  class="btn btn-primary m-b-md">Blog Mercantaile Listele</a>
							...
							<input type="text" class="form-control" id="floatingInput5" name="yazi3" placeholder="Title" value="<?=$guncelle['yazi3']?>">
							...
							<div class="col-md-4">
							<div class="form-floating">
							<input type="text" class="form-control" id="floatingInput5" name="yazi4" placeholder="Descriptions" value="<?=$guncelle['yazi4']?>">
							...
							<div class="col-md-4">
							<div class="form-floating">
							<input type="text" class="form-control" id="floatingInput5" name="yazi5" placeholder="Keywords" value="<?=$guncelle['yazi5']?>">
							<label for="floatingInput5">Keywords</label>
									...
									<div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="sira" placeholder="Blog Order" value="<?=$guncelle['sira']?>">
                                        <label for="floatingInput">Blog Order</label>
                                      </div>
                                      
									  <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="yazi2" placeholder="Button Name" value="<?=$guncelle['yazi2']?>">
                                        <label for="floatingInput">Blog Name</label>
                                      </div> 

									  <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Blog Title" value="<?=$guncelle['adi']?>">
						...
						<div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="yazi1" placeholder="Blog Fiyatı" value="<?=$guncelle['yazi1']?>">
                                        <label for="floatingInput">Button Name</label>
                                      </div>
                                   
									  <!-- 
									  <div class="mb-3">
                                        <label for="formFile" class="form-label">Paylaşan Kişinin Resmi</label>
							   ...
							   <option value="0" selected>Blog Categoriesi Seç </option>
							   <?php
					  ...
					  <?php }}?>
									...
									-->
									  ...
									  <div class="mb-3">
                                        <label for="formFile" class="form-label">Blog Picture</label>
