...
$seo= seflink($site_title);
	...
	{
		$duzenlenecek_id = 1;
		$ayar_kaydi = $db->query("SELECT * FROM yonetim WHERE id = '$duzenlenecek_id'")->fetch(PDO::FETCH_ASSOC);
	...
	$resim_tmp9 = $_FILES['resim']['tmp_name'];
	...
	$ekle  = $db->prepare("update yonetim set  renk2=:renk2,renk=:renk,resim=:resim,footer_copyright=:footer_copyright,site_title=:site_title,site_description=:site_description,site_author=:site_author,site_meta=:site_meta,site_keyword=:site_keyword,logo=:logo,favicon=:favicon,footer_logo=:footer_logo where id=:id");
	
	$simdi = $ekle->execute(array("renk2"=>$renk2,"renk"=>$renk,"resim"=>$resim,"footer_copyright"=>$footer_copyright,"site_title"=>$site_title,"site_meta"=>$site_meta,"site_description"=>$site_description,"site_author"=>$site_author,"site_keyword"=>$site_keyword,"logo"=>$logo,"footer_logo"=>$footer_logo,"favicon"=>$favicon,"id"=>$id));
...
if($simdi){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Panel Ayarlar Başarıyla Güncellendi!</strong> 
...
$guncelle = $db->query("select * from yonetim where id='1'")->fetch(PDO::FETCH_ASSOC);

$lisans = $guncelle['renk'];
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-body" style="overflow:scroll;">
                                    <h5 class="card-title">Panel Ayarlar</h5>
                                    ...
                                    <div class="mb-3">
                                        <label for="floatingInput">Site Kapansın mı ?</label>
                                        <select class="form-select" name="renk" >
                                            <?php if($lisans==1) {?>
                                            <option value="1" selected>Evet</option>
                                            <option value="2" >Hayır</option>
                                            <?php } else if($lisans==2){?>
                                    ...
                                    <div class="form-floating mb-3">
                                       ...
                                       <div class="form-floating mb-3">
                                        ...
                                        <input class="form-control" type="file" name="resim" id="formFile">
                                         ...
                                         <img src="resimler/<?=$guncelle['resim']?>" width="100">
                                      </div>
                                      
                                        <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="renk2" placeholder="Footer Linki " value="<?=$guncelle['renk2']?>">
                                        <label for="floatingInput">Footer Linki</label>
                                      </div>
                                      
                                    
                                        <div class="mb-3">
                                        <label for="formFile" class="form-label">Çıkış Logo</label>
                                       ...
                                       <div class="mb-3">
                                        <label for="formFile" class="form-label">Giriş Logo</label>
