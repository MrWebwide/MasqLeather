...
$klasord="resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into urun_kategori set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from urun_kategori where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
    ...
    <body>
	<script>
    function turkceKarakterleriDuzelt(metin) {
    // Türkçe karakterleri düzelten harita
    var turkceHarfler = {
        'ğ': 'g', 'Ğ': 'G',
        'ü': 'u', 'Ü': 'U',
        'ş': 's', 'Ş': 'S',
    ...
    // Metindeki Türkçe karakterleri ve boşlukları değiştir
    var duzeltilmisMetin = metin.replace(/[ğüşıöçĞÜŞİÖÇ ]/g, function(karakter) {
        return turkceHarfler[karakter] || (karakter === ' ' ? '+' : karakter);
    });
...
function updateDurum() {
    var adiInput = document.getElementById('floatingInput1');
    var durumInput = document.getElementsByName('durum')[0];

    var adiValue = adiInput.value.trim();

    var urlUyumluMetin = turkceKarakterleriDuzelt(adiValue);

    // Boşlukları + ile değiştir
    urlUyumluMetin = urlUyumluMetin.replace(/ /g, '+');

    durumInput.value = "bagpurses-category.php?kategori=" + urlUyumluMetin;
}
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Ürün Kategori  Ekle</h5>
									<a href="urun-kategori-listele.php"  class="btn btn-primary m-b-md">Kategori Listele</a>
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">Ürün Kategori sırası</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput1" name="adi" oninput="updateDurum()" placeholder="Kategori Adı" value="<?=$guncelle['adi']?>">
                                        <label for="floatingInput">Ürün Kategori Adı</label>
                                      </div>
