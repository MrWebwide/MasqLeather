...
$stmt = $db->prepare("SELECT * FROM jewe_img WHERE urun_id = ?");
$stmt->execute([$id]);
$resimler = $stmt->fetchAll();

$stmt = $db->prepare("SELECT stock FROM jewe WHERE id = ?");
$stmt->execute([$id]);
...
$productOptions = masq_get_product_options($db, (int) $id, 'jewelry');
